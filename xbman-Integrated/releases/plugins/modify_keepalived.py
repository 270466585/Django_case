...
class modify_keep(object):
    def __init__(self,settings,page,pages,action):
        self.__lvshostip = settings[0].IP
        self.__port = 22
        ...
        if page == '0':
            ...
            self.__cmd = 'sudo /usr/bin/python2.6 %smodify_keep.py %s %s'%(settings[0].ScriptLocation,' '.join(settings[0].onegroup.split(',')),action)
        elif page == '2':
            self.__cmd = 'sudo /usr/bin/python2.6 %smodify_keep.py %s %s'%(settings[0].ScriptLocation,' '.join(settings[0].twogroup.split(',')),action)
        elif page == '3':
